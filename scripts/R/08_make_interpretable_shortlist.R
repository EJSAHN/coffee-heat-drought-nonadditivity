...
outdir <- "results/analysis_tables/tables"
...
readr::write_tsv(interpretable, file.path(outdir, "analysis_all_candidate_panel_ranked.tsv"))
readr::write_tsv(interpretable_non_other, file.path(outdir, "analysis_interpretable_candidate_panel.tsv"))
readr::write_tsv(top_by_module, file.path(outdir, "analysis_top5_candidates_per_module.tsv"))
readr::write_tsv(top_overall, file.path(outdir, "analysis_top25_candidates_per_stage.tsv"))
readr::write_tsv(sig_modules, file.path(outdir, "analysis_module_enrichment_ranked.tsv"))
readr::write_tsv(classes, file.path(outdir, "analysis_nonadditivity_class_numbers.tsv"))

message("DONE candidate shortlist tables")