...
qcbase <- "results/analysis_tables/qc"
outdir <- "results/analysis_tables"
dir.create(outdir, recursive = TRUE, showWarnings = FALSE)

classes <- read_tsv(file.path(base, "analysis_nonadditivity_class_numbers.tsv"), show_col_types = FALSE)
enrich  <- read_tsv(file.path(base, "analysis_module_enrichment_ranked.tsv"), show_col_types = FALSE)
cand    <- read_tsv(file.path(base, "analysis_interpretable_candidate_panel.tsv"), show_col_types = FALSE)
topmod  <- read_tsv(file.path(base, "analysis_top5_candidates_per_module.tsv"), show_col_types = FALSE)
...
write_tsv(hc, file.path(base, "analysis_high_confidence_candidates.tsv"))
write_tsv(hc_top, file.path(base, "analysis_high_confidence_top3_per_module.tsv"))
write_tsv(stage_summary, file.path(base, "analysis_stage_nonadditive_totals.tsv"))
write_tsv(class_wide, file.path(base, "analysis_class_summary_long.tsv"))
write_tsv(enrich_sig, file.path(base, "analysis_significant_or_suggestive_modules.tsv"))
write_tsv(qc_low, file.path(qcbase, "analysis_low_mapping_samples_lt50.tsv"))

md <- c(
"# Coffee heat-drought non-additivity analysis digest",
"",
"## Main message",
...
"- Primary pattern: genotype-specific non-additive heat-drought response.",
"- Numeric summary: Icatu T37 burst versus larger CL153 T42/REC14 counts in the primary all-stage model.",
"- Functional summary: CL153 REC14 photosynthesis enrichment.",
"- Candidate summary: Icatu carbon transport, thylakoid assembly, chaperone, autophagy, vacuolar trafficking, and ubiquitin/ERAD axes.",
"- QC summary: CL153 low-mapping samples are retained in the primary model and evaluated by validation analyses."
)

writeLines(md, file.path(outdir, "analysis_digest.md"))

message("DONE analysis digest")